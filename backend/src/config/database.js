...
function getSafeHostLabel(connStr) {
  try {
...
console.log(`[DB] Attempting connection to: ${getSafeHostLabel(connectionString)}`);
...
pool.connect()
  .then(client => {
    console.log(`[DB] Database connection verified (${getSafeHostLabel(connectionString)})`);
    client.release();
  })
  .catch(err => {
