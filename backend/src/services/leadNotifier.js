 ...
 * high-value intents are detected (booking, emergency, escalation).
 */
...
/**
 * Notify business owner of a high-value lead with scored transcript
 * @param {Object} params
...
async function notifyOwnerOfLead({ conversationId, siteId, intent }) {
  try {
    ...
    // Optional: store lead score in DB
    await pool.query(
      ...
      console.warn('[LeadNotifier] Could not store lead_score (DB column may not exist):', err.message);
    });
